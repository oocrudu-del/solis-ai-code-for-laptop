import cv2
import time
import json
import os
import datetime
import threading
import base64
import requests
from dotenv import load_dotenv
from ai_core import ai_response
from api import GEMINI_API_KEY, GROQ_API_KEY
from groq import Groq
# तपाईंको Config र Prompt इम्पोर्ट

from prompt import SYSTEM_PROMPT, prompt


API_KEY = GEMINI_API_KEY
client = Groq(api_key=GROQ_API_KEY)
#==========================================
# 0. GEMINI API SETUP
#==========================================
GEMINI_URL = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key={API_KEY}"
HEADERS = {"Content-Type": "application/json"}
JSON_FILE = "data.json"
IMAGE_FILE = "photo.png"
is_running = True

#==========================================
# 1. VISION AGENT (क्यामेराको काम गर्ने भाग)
#==========================================
def save_to_json(ai_answer):
    if os.path.exists(JSON_FILE):
        try:
            with open(JSON_FILE, "r") as f:
                data = json.load(f)
        except:
            data = []
    else:
        data = []
        
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    data.append({"timestamp": current_time, "ai_response": ai_answer})
    
    # ५ वटा मात्र राख्ने नियम हटाएर ५० वटा सम्म राख्ने बनाइयो (लगातार चन्लको लागि)
    data = data[-100:] 

    with open(JSON_FILE, "w") as f:
        json.dump(data, f, indent=4)

def ask_vision_ai(image_path):
    with open(image_path, "rb") as image_file:
        image_b64 = base64.b64encode(image_file.read()).decode("utf-8")
        
    # क्यामेरा एआईको लागि विषेश कडा निर्देशन (यसले हातको सामान र अनुहारको डिटेल दिन्छ)
    ENHANCED_VISION_PROMPT = f"""
    {prompt}
    [CRITICAL INSTRUCTION FOR VISION AI]:
    Analyze the image in EXTREME DETAIL. You MUST include:
    1. The person's physical appearance, clothing color/type, and exact facial expression (happy, sad, neutral).
    2. EXACTLY what objects are visible, especially WHAT IS IN THEIR HANDS (e.g., phone, cup, pen, nothing).
    3. The background and environment.
    Provide a highly descriptive summary so a text AI can answer questions about the user's looks and objects accurately.
    """

    response = client.chat.completions.create(
        model="meta-llama/llama-4-scout-17b-16e-instruct", 
        messages=[{
            "role": "user",
            "content": [
                {"type": "text", "text": ENHANCED_VISION_PROMPT}, 
                {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{image_b64}"}}
            ]
        }]
    )
    return response.choices[0].message.content

def vision_worker():
    global is_running
    cap = cv2.VideoCapture(0)
    
    while is_running:
        ret, frame = cap.read()
        if not ret:
            time.sleep(2)
            continue
        
        cv2.imwrite(IMAGE_FILE, frame)
        try:
            result = ask_vision_ai(IMAGE_FILE)
            save_to_json(result)
        except Exception as e:
            print(f"\n[Background Vision Error]: {str(e)}\n")
            
        if os.path.exists(IMAGE_FILE):
            os.remove(IMAGE_FILE)
            
        # १० सेकेन्डको सट्टा ५ सेकेन्ड पर्खिने बनाइयो (छिटो रेस्पोन्सको लागि)
        for _ in range(5):
            if not is_running:
                break
            time.sleep(1)

    cap.release()

#==========================================
# MEMORY SYSTEM (memory.json म्यानेजमेन्ट)
#==========================================
MEMORY_FILE_PATH = "memory.json"

def load_chat_memory() -> list:
    """memory.json फाइलबाट पुराना सबै संवादहरू लोड गर्छ"""
    if os.path.exists(MEMORY_FILE_PATH):
        try:
            with open(MEMORY_FILE_PATH, "r", encoding="utf-8") as f:
                return json.load(f)
        except Exception as e:
            print(f"[Memory Error]: Error loading memory.json: {e}")
            return []
    return []

def save_chat_memory(history: list):
    """पुरा संवादको इतिहासलाई memory.json मा सेभ गर्छ"""
    try:
        with open(MEMORY_FILE_PATH, "w", encoding="utf-8") as f:
            json.dump(history, f, ensure_ascii=False, indent=4)
        print(f"[Memory Saved]: Conversation written to {MEMORY_FILE_PATH} ({len(history)} messages).")
    except Exception as e:
        print(f"[Memory Error]: Error saving to memory.json: {e}")

def get_formatted_past_history() -> str:
    """पुराना संवादहरूलाई LLM ले सजिलै बुझ्ने ढाँचामा ढाल्छ"""
    history = load_chat_memory()
    if not history:
        return "No previous conversation history stored yet."
    
    # पछिल्लो ५० वटा सम्मको संवादलाई प्रसंग (Context) को रूपमा तयार पार्ने
    formatted = ""
    for turn in history[-50:]:
        role = "User" if turn["role"] == "user" else "AI"
        formatted += f"{role}: {turn['content']}\n"
    return formatted

#==========================================
# 2. CHAT AGENT (च्याट गर्ने भाग)
#==========================================
def get_camera_context():
    if not os.path.exists(JSON_FILE):
        return "NO_DATA: The camera is still capturing or initializing."
    try:
        with open(JSON_FILE, "r") as f:
            data = json.load(f)
        if not data:
            return "NO_DATA: Camera data is currently empty."
        
        # पछिल्लो ३ वटा (अझै धेरै) डिटेल लिने
        recent_logs = data[-3:]
        context = ""
        for log in recent_logs:
            context += f"- {log['ai_response']}\n"
        return context
    except:
        return "NO_DATA: Error reading camera data."

# सुरुमा खाली राख्ने (memory.json बाट लोड हुनेछ)
chat_history = [{"role": "system", "content": SYSTEM_PROMPT}]

def get_ai_response(user_input):
    global chat_history
    camera_data = get_camera_context()
    
    # १. memory.json बाट पुराना सबै संवादहरू र प्रसंग लोड गर्ने
    history = load_chat_memory()
    past_history = get_formatted_past_history()

    # २. मुख्य निर्देशनसँगै पुराना संवादको इतिहास र क्यामेराको विवरण मिलाउने
    # यहाँ कडा नियम थपिएको छ जसले AI लाई पहिले memory.json पढ्न बाध्य बनाउँछ
    SMART_INSTRUCTIONS = f"""
    {SYSTEM_PROMPT}
    
    [PAST CONVERSATION HISTORY FROM MEMORY.JSON]:
    {past_history}
    
    [CURRENT VISUAL CONTEXT FROM CAMERA]:
    {camera_data}
    
    [CRITICAL MEMORY RETRIEVAL RULES]:
    1. READ THE [PAST CONVERSATION HISTORY] FIRST: When the user asks a question, your absolute first priority is to scan the previous conversation history from the memory file.
    2. RETRIEVE PAST DETAILS: If they ask about something they told you earlier (e.g., "bholi ma kaha jaane?", "what did I say about school?", "do you remember my name?"), retrieve the exact details of what you both discussed in the history. Formulate a highly personalized, friendly Nepali reply (using "बोस" as requested) blending those details perfectly.
    3. LLM FALLBACK: If the answer, detail, or topic is NOT in the conversation history, then answer normally using your own general knowledge (LLM). Do not invent false memories.
    
    [CRITICAL BEHAVIOR RULES]:
    You are an active, highly empathetic personal AI assistant.
    VISUAL QUESTIONS: If the user asks "How do I look?", "What is in my hand?", or about their surroundings, READ THE [CURRENT VISUAL CONTEXT] and answer accurately based ONLY on that.
    NO DATA HANDLING: If the visual context says "NO_DATA", DO NOT stay blank. Reply politely: "बोस, मेरो क्यामेरा खुल्दै छ, एकैछिन पर्खिनुहोस् ल!".
    EMOTION: If the context says the user looks sad, ask: "के भयो बोस, किन दुखी हुनुहुन्छ?".
    LANGUAGE: Always respond in the language the user is speaking.
    """
    
    # ३. कमान्ड इतिहासमा नयाँ प्रयोगकर्ताको भनाइ थप्ने
    history.append({"role": "user", "content": user_input})
    
    # जेमिनाईको एक्टिभ कुराकानीको लागि पछिल्लो १० संवाद मात्र पठाउने (टोकन बचाउनको लागि)
    active_history = history[-10:]
    gemini_contents = []
    for msg in active_history:
        role = "user" if msg["role"] == "user" else "model"
        gemini_contents.append({
            "role": role,
            "parts": [{"text": msg["content"]}]
        })
        
    payload = {
        "system_instruction": {
            "parts": [{"text": SMART_INSTRUCTIONS}]
        },
        "contents": gemini_contents
    }
    
    try:
        response = requests.post(GEMINI_URL, headers=HEADERS, data=json.dumps(payload))
        response_data = response.json()
        
        if response.status_code == 200:
            ai_answer = response_data['candidates'][0]['content']['parts'][0]['text']
        else:
            ai_answer = ai_response(user_input)  # फेल भएमा पुरानो AI प्रयोग गर्ने

        if not ai_answer or ai_answer.strip() == "":
            ai_answer = "माफ गर्नुहोला बोस, मैले ठ्याक्कै बुझ्न सकिनँ। फेरि भन्नुहुन्छ कि?"

        # ४. एआईको जवाफलाई पनि इतिहासमा थप्ने र memory.json मा सँगै सेभ गर्ने
        history.append({"role": "assistant", "content": ai_answer})
        save_chat_memory(history)
        
        # पुराना कलिङ कोडहरू बिग्रिन नदिन global chat_history लाई सँगै अपडेट राख्ने
        chat_history = [{"role": "system", "content": SMART_INSTRUCTIONS}] + history[-10:]
        
        return ai_answer
    except Exception as e:
        # कतै एरर आएमा थपिएको युजर मेसेज हटाउने
        if history and history[-1]["role"] == "user":
            history.pop()  
        return f"Error: {str(e)}"

#==========================================
# 3. MAIN RUNNER

