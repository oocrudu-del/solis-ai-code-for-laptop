...
import pyperclip
import cv2
import numpy as np  # <-- यहाँ NumPy इम्पोर्ट गरियो (यो छुटेको थियो)
...
# ================== PALETTE SETUP (GLOWING COLORFUL SOLAR MATRIX) ==================
...
CARD_WIDTH = 250
CARD_HEIGHT = 80
CAM_INDEX = 0             # <-- यहाँ क्यामेरा इन्डेक्स सेट गरियो (यो छुटेको थियो)
...
# Ensure the Project subdirectory exists
...
class IRIS_NeuralInterface:
    ...
    def _open_search_browser(self):
        ...
        def run_pyqt_app():
            try:
                ...
                print(f"Error launching PyQt5 Search Window: {e}")
    ...
    def _draw_radar_animation(self):
        ...
        # हात माउस एक्टिभ छ र फ्रेम उपलब्ध छ भने पहिले क्यामेरा फिड सेन्टरमा कोर्ने
        if self.hand_mouse_active and self.latest_hand_frame is not None:
            try:
                ...
                if canvas_w > 10 and canvas_h > 10:
                    ...
                    imgtk = ImageTk.PhotoImage(image=img_resized)
                    self.center_orb_canvas.imgtk = imgtk  # keep reference
        ...
        # राडार एनिमेसनलाई क्यामेरा फिडको माथि ओभरले गर्ने (Sci-Fi Look)
    ...
    def _commit_note_to_bank(self):
        ...
        try:
            ...
            messagebox.showerror("IO System Error", f"Could not perform physical file write: {e}")
    ...
    def _toggle_hand_mouse(self):
        ...
        if not self.hand_mouse_active:
            ...
            # थ्रेड चलाएर ब्याकग्राउन्डमा प्रोसेस सुचारु गर्ने
            ...
            with mp_hands.Hands(
                ...
                while self.hand_mouse_active and cap.isOpened():
                    ...
                    if res.multi_hand_landmarks:
                        ...
                        # माउस एक्सनहरू सुरक्षित रूपमा चलाउने
                        try:
                            if base == G.DRAG:
                                gesture = G.DRAG
                            ...
                            print("[Hand Mouse] FailSafe Triggered. Mouse went to corner.")
                            self.hand_mouse_active = False
                            break

                        # हत्केलाको बीचमा कर्सर डट कोर्ने
                    ...
                    # मेन-थ्रेड GUI को लागि नयाँ फ्रेम थ्रेड-सेफ कपी गरेर सेभ गर्ने
